...
public class RulesOf6005 {
    ...
    public static boolean mayUseCodeInAssignment(boolean writtenByYourself,
        ...
        if (writtenByYourself) {
            return true; // Code written by the user is always allowed
        }
        if (writtenAsCourseWork) {
            return false; // Coursework not cited is not allowed
        }

        // If the code is available to others and not required to be implemented,
        // then citation matters.
        if (availableToOthers && !implementationRequired) {
            return citingYourSource; // Return true if cited
        }

        return false; // In all other cases, not allowed
    }
